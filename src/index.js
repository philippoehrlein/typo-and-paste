...
import TypoDialog from "./components/TypoDialog.vue";
...
window.panel.plugin("philippoehrlein/typo-and-paste", {
  ...
  components: {
    'k-typo-and-paste-dialog': TypoDialog
  },
  ...
  use: [
    ...
    function (Vue) {
      Vue.mixin({
        mounted() {
          if (this.$options.name === "k-writer") {
            Vue.nextTick(() => {
              const addButton = (toolbar) => {
                if (toolbar && !toolbar.querySelector('.typo-button')) {
                  const button = new Vue({
                    render: (h) => h('button', {
                      class: 'k-button k-toolbar-button typo-button',
                      attrs: {
                        'data-has-icon': true,
                        'title': 'Typo & Paste',
                        'type': 'button'
                      ...
                      h('span', { class: 'k-button-icon' }, [
                        h('svg', {
                          ...
                          h('use', {
                            attrs: {
                              'xlink:href': '#icon-typo-and-paste'
                            }
                          })
                        ])
                      ])
              ...
              const staticToolbar = this.$el.querySelector('nav.k-toolbar.k-writer-toolbar');
              if (staticToolbar) {
                addButton(staticToolbar);
              }

              const observer = new MutationObserver((mutations) => {
                const inlineToolbar = this.$el.querySelector('nav.k-toolbar.k-writer-toolbar[data-inline="true"]');
                if (inlineToolbar) {
                  addButton(inlineToolbar);
                }
              });
